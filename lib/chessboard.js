...
;(function () {
  ...
  // pieces
  var PIECES_WHITE = { king: 'wK', queen: 'wQ', rook: 'wR', bishop: 'wB', knight: 'wN', pawn: 'wP' }
  var PIECES_BLACK = { king: 'bK', queen: 'bQ', rook: 'bR', bishop: 'bB', knight: 'bN', pawn: 'bP' }
  var PIECES_STRATEGY_COLOR = { yellow: 'cY', blue: 'cL', green: 'cG', red: 'cE', orange: 'cO' }
  var PIECES_STRATEGY_DIRECTION = { N: 'dT', NE: 'dH', E: 'dA', SE: 'dD', S: 'dS', SW: 'dU', W: 'dW', NW: 'dC' }
  ...
  CSS['strategyPiecesColor'] = 'strategy-pieces-color-7492d'
  CSS['strategyPiecesDirection'] = 'strategy-pieces-direction-7492d'
  ...
  function validPieceCode (code) {
    return isString(code) && code.search(/^[bwcd][KQRNBPYLGEOTHADSUWC]$/) !== -1
  }
  ...
  function validFen (fen) {
    ...
    for (var i = 0; i < 8; i++) {
      if (chunks[i].length !== 8 ||
          chunks[i].search(/[^kqrnbpKQRNBPylgeothadsuwc1]/) !== -1) {
        return false
      }
    }
    return true
  ...
  function fenToPieceCode (piece) {
    ...
    // this does not want to get from GLOBAL vars at top. INVESTIGATE
    var PIECES_WHITE = { king: 'wK', queen: 'wQ', rook: 'wR', bishop: 'wB', knight: 'wN', pawn: 'wP' }
    var PIECES_BLACK = { king: 'bK', queen: 'bQ', rook: 'bR', bishop: 'bB', knight: 'bN', pawn: 'bP' }
    var PIECES_STRATEGY_COLOR = { yellow: 'cY', blue: 'cL', green: 'cG', red: 'cE', orange: 'cO' }
    var PIECES_STRATEGY_DIRECTION = { N: 'dT', NE: 'dH', E: 'dA', SE: 'dD', S: 'dS', SW: 'dU', W: 'dW', NW: 'dC' }
    ...
    // All of these needs to be in a single loop

    Object.values(PIECES_WHITE).forEach(function (value) {
      if (piece === value[1]) {
        peace_code = value
      }
    })

    Object.values(PIECES_BLACK).forEach(function (value) {
      if (piece === value[1].toLowerCase()) {
        peace_code = value
      }
    })

    Object.values(PIECES_STRATEGY_COLOR).forEach(function (value) {
      if (piece === value[1].toLowerCase()) {
        peace_code = value
      }
    })

    Object.values(PIECES_STRATEGY_DIRECTION).forEach(function (value) {
      if (piece === value[1].toLowerCase()) {
        peace_code = value
      }
    })
  ...
  function buildContainerHTML (hasSparePieces) {
    ...
    html += '<details><summary class="btn btn-sm btn-outline-secondary">Strategy Pieces</summary>'
    html += '<div class="{strategyPiecesColor}"></div>'
    html += '<div class="{strategyPiecesDirection}"></div>'
    html += '</details>'
    html += '</div>'
  ...
  function constructor (containerElOrString, config) {
    ...
    var $strategyPiecesColor = null
    var $strategyPiecesDirection = null
    ...
    function buildPieceHTML (piece, hidden, id) {
      ...
      html += 'alt="" ' +
        'class="{piece}" ' +
        'data-piece="' + piece + '" ' +
        'style="width:' + squareSize + 'px;' + 'height:' + squareSize + 'px; padding: 1px;'
    ...
    function buildSparePiecesHTML (type) {
      var pieces = PIECES_WHITE
      if (type === 'black') {
        pieces = PIECES_BLACK
      } else if (type === 'strategy_color') {
        pieces = PIECES_STRATEGY_COLOR
      } else if (type === 'strategy_direction') {
        pieces = PIECES_STRATEGY_DIRECTION
      }

      var html = ''

      for (const [key, value] of Object.entries(pieces)) {
        sparePiecesElsIds[value] = value + '-' + uuid()
        html += buildPieceHTML(value, false, sparePiecesElsIds[value])
      }
    ...
    function drawBoard () {
      ...
      if (config.strategyPieces) {
        $strategyPiecesColor.html(buildSparePiecesHTML('strategy_color'))
        $strategyPiecesDirection.html(buildSparePiecesHTML('strategy_direction'))
      }
    }
    ...
    function mousemoveWindow (evt) {
      if (isDragging) {
        ...
        // do not start selecting text while moving color blocks
    ...
    function addEvents () {
      ...
      $container.on('mousedown', '.' + CSS.strategyPiecesColor + ' .' + CSS.piece, mousedownSparePiece)
      $container.on('mousedown', '.' + CSS.strategyPiecesDirection + ' .' + CSS.piece, mousedownSparePiece)
    ...
    function initDOM () {
      ...
      if (config.sparePieces) {
        ...
        $strategyPiecesColor = $container.find('.' + CSS.strategyPiecesColor)
        $strategyPiecesDirection = $container.find('.' + CSS.strategyPiecesDirection)
      }
